...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    ;;
    * )
      if [ "$userInterfaceNameInput" == "" ]; then
        userInterfaceNameInput="$1"
      fi
    ;;
  esac
...
if [ "$userInterfaceNameInput" != "" ]; then
    ...
    interface=$userInterfaceNameInput
...
arrayOfInterfaces=$(ifconfig | cut -d ' ' -f1| tr ':' '\n' | awk NF)

for interface in $arrayOfInterfaces; do
